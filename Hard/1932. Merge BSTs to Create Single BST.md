...
##  Core Idea
1. **Dictionary of roots:** Map each root value → TreeNode.  
...
- **DFS merge:** O(total nodes) ≤ O(N), since each tree has ≤ 3 nodes.  
...
- Roots: {2 → [2,1], 3 → [3,2,5], 5 → [5,4]}  
- Leaves: {1,2,5,4}  

### Step 2: Find global root
- Root candidates: 2, 3, 5  
- Leaves contain 2 and 5 → only `3` is not a leaf → global root = 3.  

### Step 3: DFS merge
- Start at root 3.  
- Left child = 2 → matches root of tree [2,1]. Merge.  
- Right child = 5 → matches root of tree [5,4]. Merge.  

### Step 4: Validate BST
- Inorder traversal = [1,2,3,4,5] → strictly increasing.  
- All trees used.  
