...
## 📘 Problem Summary
We are given:
- An array `stones[]` representing piles of stones.
- An integer `k` representing the number of consecutive piles that can be merged in one move.


Rules:
- In each move, you must merge **exactly `k` consecutive piles** into one.
- The cost of a merge = sum of stones in those `k` piles.
- The goal is to merge all piles into a single pile with **minimum total cost**.
- If it is **impossible** to end up with one pile, return `-1`.

---

## 🔑 Core Idea
- **Feasibility check:**  
  To reduce `n` piles to 1, the number of merges must align with `(n - 1) % (k - 1) == 0`.  
  If not, return `-1`.

- **Dynamic Programming (DP):**  
  - Define `dp[i, j]` = minimum cost to merge piles from index `i` to `j`.  
  - Transition: split interval `[i..j]` into two parts and merge recursively.  
  - Add cost of merging into one pile only when the number of piles in `[i..j]` can be reduced to 1.

- **Prefix sums:**  
  Used to quickly calculate the sum of stones in any interval, since merging cost depends on total stones.

- **Final answer:**  
  `dp[0, n-1]` → minimum cost to merge the entire array into one pile.
