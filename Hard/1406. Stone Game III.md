...
- **Minimax DP**:  
  ...
  - **Alice maximizes** her advantage  
  - **Bob minimizes** Alice’s advantage (i.e., maximizes his own)  
  - **Relative score** = `Alice's total - Bob's total`  
  - **Why relative?** → No need to track two scores → simpler logic

- **Rolling Array**: `dp[i % 3]` → **O(1) space**  
  - We only need `dp[i+1]`, `dp[i+2]`, `dp[i+3]` to compute `dp[i]`  
  - Since max take = 3 → **only last 3 states matter**  
  - `i % 3` cycles through indices `0,1,2` → overwrites old values safely
