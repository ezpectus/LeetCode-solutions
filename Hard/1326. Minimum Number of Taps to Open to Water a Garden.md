...
2. Traverse from `0` to `n`:  
   ...
   - If at any point `currEnd < i` → impossible, return `-1`.  
...
1. **Clamp intervals to [0, n]**  
   - Each tap’s coverage must be restricted to the garden boundaries.  
   - Example: tap at position 0 with range 3 → covers `[0,3]`, not `[-3,3]`.
...
4. **Edge case: n = 0**  
   - Garden length is zero → no taps needed.  
   - Answer is `0`.
